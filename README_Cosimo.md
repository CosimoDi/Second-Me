...
2.  **专家矩阵: 垂类 Agent (The Solvers)**
    ...
    *   **协作流**：Keeper 将包含“用户背景 + 简历 + 亮点”的 Context 包发送给 Solver C (面试教练) -> Solver C 执行模拟面试 -> Solver C 返回评分报告 -> Keeper 将报告存入长期记忆。
...
*   **流程**：
    ...
    2.  AI 结合公司现有的**专业评测问卷结果**与日常日志，基于**冰山模型**（知识技能、通用能力、隐性特质）生成多维度的“成长轨迹报告”。
    3.  系统识别学员在冰山水面下的深层需求，主动推送个性化的补充材料或下一步行动建议。
