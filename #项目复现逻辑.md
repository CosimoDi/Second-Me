...
如果哪一步搞错了以至于要全重新搞，build这一步就需要Building 2597.4s，后面跑一遍模型训练差不多是1小时（没报错的情况下），train这一步大概4个小时。改bug最好还是要谨慎。————由于不知道为什么，我之前用GPU加速好像失败了，CUDA一直检测不到，所以是CPU跑的train
...
| **本地登录** | `SSL: CERTIFICATE_VERIFY_FAILED` | **原因**: `app.secondme.io` 证书过期。**修复**: 在 [lpm_kernel/api/domains/upload/client.py](lpm_kernel/api/domains/upload/client.py) 的 `count_uploads()` 中为 `requests.get()` 添加 `verify=False`，绕过证书校验。 |
...
| **Train** | `return code: 137` / 日志显示 `Killed` | **原因**: 训练进程被系统 OOM Killer 杀死（内存不足）。<br>**修复**: 已在 [lpm_kernel/L2/train_for_user.sh](lpm_kernel/L2/train_for_user.sh) 中增加 CPU 训练降载逻辑（自动把 `max_seq_length` 降到 1024、`batch_size` 降到 1、`gradient_accumulation_steps` 设为 1）。<br>**如果仍失败**: 增加 Docker/WSL 内存分配，或启用 CUDA/量化后再试。 |

### ✅ 训练 OOM 的实际修复流程（已验证生效）
**现象**: 前端启动训练仍用旧脚本参数（`max_seq_length=2048`、`batch_size=2`），容器内脚本未生效，导致再次 `Killed/137`。  
**根因**: `docker-compose.yml` 未挂载 `lpm_kernel`，容器继续使用镜像内旧脚本。  

**修复步骤**:
1. 在 [docker-compose.yml](docker-compose.yml) 的 backend `volumes` 中新增挂载：
    - `./lpm_kernel:/app/lpm_kernel`
    - `./scripts:/app/scripts`
2. 重新创建后端容器，使挂载生效：
    - `docker compose up -d backend`
3. 通过后端 API 触发训练（确保使用新脚本）：
    - `POST http://localhost:8002/api/trainprocess/start`

**结果**: 训练参数自动降载生效，日志出现 `batch size=1`、`gradient_accumulation_steps=1`，训练进入正常迭代。（前端无法开始训练是正常的，因为后端已经启动了一个训练。）
